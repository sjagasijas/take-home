...
function register_brewery_cpt(){
	register_post_type('brewers',array (
		'label' => 'Brewers',
		'public' => true,
...
add_action( 'update_brewery_list', 'get_brewery' );

function get_brewery(){
	
$request = wp_remote_get( 'https://api.openbrewerydb.org/breweries' );
...
if(!empty( $data ) ) {
	...
	foreach( $data as $brew ) {
		...
		$brew_slug = sanitize_title($brew->name . '-' . $brew->id);
		$brew_name = sanitize_title($brew->name);
		
		$existing_brew = get_page_by_path($brew_slug, 'OBJECT', 'brewers');
		if($existing_brew === null){
		
		//insert post in brewers post type
		$inserted_brewers = wp_insert_post([
		'post_name' => $brew_slug,
		'post_title' => $brew_name,
		'post_type' =>'brewers',
		'post_status' => 'publish'
		]);
		...
		if( is_wp_error($inserted_brewers)){
			continue;
		}
		
		$fillable = [
		'field_61f29865c6afa' => 'name',
		'field_61f29894c6afc' => 'brewery_type',
		'field_61f29882c6afb' => 'street',
		'field_61f298a2c6afd' => 'city',
		'field_61f298c0c6aff' => 'phone',
		'field_61f298b6c6afe' => 'website_url',
		'field_61f2b588e02d4' => 'updated_at'
		
		];
		
		foreach ($fillable as $key => $name){
			update_field( $key, $brew->$name, $inserted_brewers );
		}
		
		
	} else{ //if existing post
	 
	   $existing_brew_id = $existing_brew->ID;
      $exisiting_brew_timestamp = get_field('updated_at', $existing_brew_id);

      if( $brew->updated_at >= $exisiting_brew_timestamp ){
		...
		foreach( $fillable as $key => $name ){
          update_field( $name, $brew->$name, $existing_brew_id);
        }
	 ...
	 if (++$i == 100) break; //break if data is equals to 100
